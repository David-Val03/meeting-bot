...
export class AzureBlobStorageProvider implements StorageProvider {
  ...
  async uploadBuffer(
    ...
    try {
      logger.info(`Starting Azure Blob buffer upload for ${blobName}`);
      await blob.uploadData(buffer, {
        blobHTTPHeaders: { blobContentType: contentType },
      });
      logger.info(`Azure Blob buffer upload complete for ${blobName}`);
      return true;
    } catch (err) {
      logger.error(
        `Azure Blob buffer upload failed for ${blobName}`,
        err as any,
      );
      return false;
    }
