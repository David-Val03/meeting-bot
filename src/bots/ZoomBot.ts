...
export class ZoomBot extends BotBase {
    ...
    try {
      ...
      if (error instanceof WaitingAtLobbyRetryError) {
        await handleWaitingAtLobbyError(
          { token: bearerToken, botId, eventId, provider: 'zoom', error },
          this._logger,
        );
      }
  ...
  > {
    ...
    await this.page.route('**/*.exe', (route) => {
      this._logger.info(
        `Detected .exe download: ${route.request().url()?.split('download')[0]}`,
      );
    });
    ...
    try {
      ...
      const acceptCookies = await this.page.locator('button', {
        hasText: 'Accept Cookies',
      });
      await acceptCookies.waitFor({ timeout: 5000 });

      this._logger.info(
        'Clicking the "Accept Cookies" button...',
        await acceptCookies.count(),
      );
    ...
    const findAndEnableJoinFromBrowserButton = async (
      retry: number,
    ): Promise<boolean> => {
      try {
        if (retry >= attempts) {
        ...
        const launchMeetingGetByRole = this.page
          .getByRole('button', { name: /Launch Meeting/i })
          .first();
        this._logger.info(
          'Does Launch Meeting exist',
          await launchMeetingGetByRole.isVisible(),
        );

        const launchDownloadGetByRole = this.page
          .getByRole('button', { name: /Download Now/i })
          .first();
        this._logger.info(
          'Does Download Now exist',
        ...
        const joinFromBrowser = await this.page
          .locator('a', { hasText: 'Join from your browser' })
          .first();
        ...
        if ((await joinFromBrowser.count()) > 0) {
          ...
          this._logger.info(
            'Try to find the Join from your browser button again...',
            retry + 1,
          );
    ...
    const visitWebClientByUrl = async (): Promise<boolean> => {
      ...
      try {
        ...
        this._logger.info('Navigating to Zoom Web Client URL...', {
    ...
    const waitForJoinFromBrowserNav = async (): Promise<boolean> => {
      try {
        ...
        const navPromise = new Promise<boolean>((foundResolver) => {
          const interv = setInterval(async () => {
            ...
            try {
              ...
              if ((await joinFromBrowser.count()) > 0) {
                this._logger.info(
                  'Waiting for zoom navigation to meeting page...',
                  params.userId,
                );
              } else {
                clearInterval(interv);
              ...
              if (e?.name === 'TimeoutError') {
                this._logger.info(
                  'Join from your browser is no longer present on page...',
                  params.userId,
                );
                clearInterval(interv);
                foundResolver(true);
              ...
              this._logger.info(
                'An error happened while waiting for zoom navigation to finish',
                e,
              );
              if (attempt >= maxAttempts) {
                clearInterval(interv);
        ...
        this._logger.info(
          'Zoom error: Unable to move forward from Join from your browser',
          params.userId,
        );
        return false;
      }
    ...
    const foundAndClickedJoinFromBrowser =
      await findAndEnableJoinFromBrowserButton(0);
    ...
    if (!foundAndClickedJoinFromBrowser || !navSuccess) {
      ...
      this._logger.info(
        'Failed to enable Join from your browser button...',
        params.userId,
      );
      this._logger.info(
        'Zoom Bot will now attempt to access the Web Client by URL...',
        params.userId,
      );
      ...
      if (!canAccess) {
        await uploadDebugImage(
          await this.page.screenshot({ type: 'png', fullPage: true }),
          'direct-access-webclient',
          params.userId,
          this._logger,
          params.botId,
        );
        throw new Error(
          'Unable to join meeting after trying to access the web client by /wc/join/',
        );
      }
    }
    ...
    const detectAppContainer = async (
      startWith: 'app' | 'iframe',
    ): Promise<boolean> => {
      try {
        ...
        apps.push(startWith);
        if (startWith === 'app') {
          const input = await this.page.waitForSelector('input[type="text"]', {
            timeout: 30000,
          });
          const join = await this.page.locator('button', { hasText: /Join/i });
          join.waitFor({ timeout: 15000 });
          this._logger.info('App container...', {
            input: input !== null,
            join: join !== null,
          });
          if (input && join) {
        ...
        if (startWith === 'iframe') {
          ...
          this._logger.info(
            'Iframe container...',
            await iframeElementHandle?.getAttribute('id'),
          );
        ...
        this._logger.info(
          'Cannot detect the App container for Zoom Web Client',
          startWith,
          err,
        );
        await uploadDebugImage(
    ...
    const foundAppContainer = await detectAppContainer(
      usingDirectWebClient ? 'app' : 'iframe',
    );

    if (!iframe || !foundAppContainer) {
      throw new Error(
        `Failed to get the Zoom PWA iframe on user ${params.userId}`,
      );
    }
      ...
      const waitAtLobbyPromise = new Promise<boolean>((resolveMe) => {
        ...
        waitInterval = setInterval(async () => {
          try {
            ...
            if (joinedText === 'participants') return;
            ...
            this._logger.info(
              'Final Number of participants while waiting...',
              num,
            );
    ...
    try {
      ...
      const notifyPromise = new Promise<boolean>((res) => {
        ...
        notifyInternval = setInterval(async () => {
          try {
            const cameraDiv = await iframe
              .locator('div', { hasText: /^Cannot detect your camera/i })
              .first();
            const micDiv = await iframe
              .locator('div', { hasText: /^Cannot detect your microphone/i })
              .first();
            ...
            if (
              micNotifications.length >= 2 &&
              cameraNotifications.length >= 2
            ) {
              clearInterval(notifyInternval);
            ...
            try {
              ...
              this._logger.info(
                'Unable to click the x notifications',
                counter,
                err,
              );
            }
    ...
    const images = await this.recordMeetingPage({ ...params });
  ...
  private async recordMeetingPage(params: JoinParams): Promise<string[]> {
