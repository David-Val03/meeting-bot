...
export class MicrosoftTeamsBot extends MeetBotBase {
  ...
  async join({
    ...
    try {
      ...
      if (_state.includes('finished') && !uploadResult) {
        ...
        this._logger.error('Recording completed but upload failed', {
          botId,
          userId,
          teamId,
        });
  ...
  private async joinMeeting({
    ...
    try {
      const warmupPage = await createBrowserContext(
        url,
        this._correlationId,
        'microsoft',
      );
      ...
      for (const selector of joinButtonSelectors) {
        try {
          ...
          this._logger.info(
            `Pre-warming: Found button with selector: ${selector}`,
          );
      ...
      this._logger.info(
        'Waiting 10 seconds before opening browser for actual meeting...',
      );
    ...
    try {
      ...
      this._logger.info(
        'Name input field not found after 120s, skipping...',
        err?.message,
      );
    }
    ...
    const dismissDeviceChecksAndNotifications = async () => {
        ...
        try {
          ...
          while (true) {
            const visibleButtons = await this.page
              .locator('button[title="Close"]:visible')
              .all();
            ...
            for (const btn of visibleButtons) {
              try {
                ...
                closeButtonsClicked++;
                this._logger.info(
                  `Clicked a "Close" button #${closeButtonsClicked}`,
                );
          ...
          // Log and ignore this error
          this._logger.info('Device permissions modals might be missing...', {
            error,
          });
        }
      };
      ...
      await deviceCheck();
      this._logger.info(
        'Finished dismissing device checks and notifications...',
      );
    };
    ...
    this._logger.info(
      'Waiting 5 seconds for audio to stabilize before recording...',
    );
  ...
  private async recordMeetingPageWithFFmpeg({
    ...
    try {
      ...
      if (!paStatus.includes('virtual_output.monitor')) {
        ...
        this._logger.info(
          'Attempting to restart PulseAudio and recreate virtual audio device...',
        );
        ...
        try {
          ...
          this._logger.info(
            'PulseAudio sources after restart:',
            newStatus.trim(),
          );
        } catch (err) {
          this._logger.error(
            'Failed to restart PulseAudio or recreate virtual audio device:',
            err,
          );
        }
      }
    ...
    try {
      ...
      this._logger.info(
        `FFmpeg recording started successfully (audioOnly: ${audioOnly})`,
      );
                ...
                const interval = setInterval(() => {
                  try {
                    ...
                    const contributors = Array.from(
                      document.querySelectorAll('button[aria-label=People]') ??
                        [],
      ...
      if (ffmpegFailed) {
        ...
        this._logger.error('Recording completed but file upload failed', {
          botId,
          eventId,
          userId,
          teamId,
        ...
        this._logger.info('Recording completed successfully ✨', {
          botId,
          eventId,
          userId,
          teamId,
